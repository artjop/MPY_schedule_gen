...
async def refresh_schedule(group_slug: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not group.schedule_json:
            group.ics_text = None # Если это первый запуск и ошибка, то календаря нетДавай разберемся по шагам. Проблема в том, что фронтенд показывает "Ожидание...", но бэкенд либо не запускает задачу, либо сразу падает с ошибкой, которую мы не видим, либо статус не успевает записаться в базу до того, как фронтенд начинает опрос.

Я полностью переписал логику обновления, чтобы она была **синхронной и надежной** для MVP.
**Что изменено:**
1.  **Убраны фоновые задачи (`BackgroundTasks`)**: Они создавали гонку состояний (race condition), когда фронтенд начинал опрашивать статус раньше, чем задача успевала записать его в БД. Теперь запрос `POST /refresh` ждет завершения парсинга. Это займет 2-5 секунд, но гарантирует корректность.
2.  **Жесткая обработка ошибок**: Любой чих парсера теперь ловится, записывается в `last_error`, сбрасывает флаг `is_fetching` и возвращается пользователю.
3.  **Логирование**: Добавлены принты в консоль сервера на каждом этапе, чтобы мы видели, где затык.
4.  **Фронтенд**: Упрощен. Кнопка блокируется, отправляет запрос, ждет ответа. Если ответ "OK" — запускает поллинг статуса для обновления времени. Если ошибка — показывает её сразу.

Вот исправленные файлы. Замени их полностью.

### 1. `main.py`
Здесь вся логика синхронного обновления с подробным логом.
